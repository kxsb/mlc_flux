...

{
  ...
  echo "=== $(date -u +%Y-%m-%dT%H:%M:%SZ) DAILY SYNC END - transactions=${TRANSACTIONS_STATUS} odoo_enrichment=${ODOO_ENRICHMENT_STATUS} individual_balances=${INDIVIDUAL_BALANCES_STATUS} ==="

  if [ "$TRANSACTIONS_STATUS" -ne 0 ] || [ "$ODOO_ENRICHMENT_STATUS" -ne 0 ] || [ "$INDIVIDUAL_BALANCES_STATUS" -ne 0 ]; then
    exit 1
  fi
} >> "$LOG_FILE" 2>&1
