...
python -m server.sync_transactions
TRANSACTIONS_STATUS=$?

python -m server.sync_odoo_professional_enrichment
...
python -m server.sync_cyclos_professional_actor_user_links \
...
python -m server.sync_cyclos_individual_daily_balances \
  --date-from "$BALANCES_DATE_FROM" \
  --date-to "$BALANCES_DATE_TO"
...
python -m server.sync_cyclos_professional_daily_balances \
  --date-from "$BALANCES_DATE_FROM" \
  --date-to "$BALANCES_DATE_TO"
