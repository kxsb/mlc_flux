...
def _resolve_holdings_period(cur, requested_start, requested_end) -> dict:
    """
    Résout la période effectivement couverte pour les indicateurs de détention.
    ...
    requested_start_iso = (
        ...
        if hasattr(requested_start, "isoformat")
        else requested_start
    )
    requested_end_iso = (
        requested_end.isoformat()
        if hasattr(requested_end, "isoformat")
        else requested_end
    )

    def _empty_bounds(source=None):
        return {
    ...
    def _table_bounds(table_name, day_column):
        try:
            row = cur.execute(
                f"""
                SELECT
    ...
    effective_start = requested_start_iso or min_date
    effective_end = requested_end_iso or max_date

    effective_start = max(effective_start, min_date)
    effective_end = min(effective_end, max_date)
...
def get_pilotage_holdings_summary(requested_start, requested_end) -> dict:
    ...
    operator_professional_refs_count = _load_operator_professional_refs_count()
    averages["operator_professional_refs_count"] = operator_professional_refs_count
    averages["operator_professional_stock_applicable"] = (
        operator_professional_refs_count > 0
    )

    def _float_or_zero(value):
        return float(value or 0.0)

    average_numeric_mass = _float_or_zero(averages.get("average_numeric_mass"))
    ...
    average_unventilated_numeric_mass_share = None

    if average_numeric_mass > 0:
        average_unventilated_numeric_mass = round(
            max(0.0, average_numeric_mass - average_ventilated_stock),
            2,
        )
        average_unventilated_numeric_mass_share = round(
            average_unventilated_numeric_mass / average_numeric_mass,
            6,
        )

    averages["average_unventilated_numeric_mass"] = (
        average_unventilated_numeric_mass
    )
    averages["average_unventilated_numeric_mass_share_of_numeric_mass"] = (
        average_unventilated_numeric_mass_share
    )
