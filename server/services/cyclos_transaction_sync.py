...
def sync_cyclos_transactions_sample(
    *,
    mlc_id: str = "graine",
    days: int = 7,
    limit: int = 100,
    ...
    raw_transactions = get_transactions(days=days)
    limited_transactions = raw_transactions[:limit]
    ...
    if write:
        if reset:
            deleted = reset_transactions_table()
...
def reset_transactions_table() -> int:
    init_db()

    conn = get_connection()
    cur = conn.cursor()

    count = cur.execute("SELECT COUNT(*) AS count FROM transactions").fetchone()["count"]
