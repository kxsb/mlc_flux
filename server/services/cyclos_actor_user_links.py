...
def _private_actor_mapping_path() -> Path:
    return _instance_data_dir() / "private_actor_mapping.json"


def _legacy_user_mapping_path() -> Path:
    return _instance_data_dir() / "user_mapping.json"


def _actor_user_links_path() -> Path:
    return _instance_data_dir() / "actor_user_links.json"
...
def save_actor_user_links(payload):
    ...
    _atomic_write_json(_actor_user_links_path(), payload)
...
def load_known_actor_pseudonyms():
    ...
    """
    private_mapping = _load_json(_private_actor_mapping_path(), default={})
    actors = {}

    items = private_mapping.get("items") if isinstance(private_mapping, dict) else None
    if isinstance(items, dict):
        for stable_key, record in items.items():
            key = str(stable_key)
            if not key.startswith("U:actor:"):
                continue

            actor_id = key.split(":", 2)[2].strip()
            if not actor_id:
                continue

            pseudonym = str((record or {}).get("label") or "").strip()
            if pseudonym:
                actors[actor_id] = pseudonym

        if actors:
            return actors

    # Fallback ancien format, au cas où une instance legacy l’utiliserait encore.
    raw_mapping = _load_json(_legacy_user_mapping_path(), default={})
...
def sync_actor_user_links(date_from: str, date_to: str, *, progress_callback=None):
    ...
    return {
        ...
        "months": months,
        **merge_result,
        "output_path": str(_actor_user_links_path()),
    }
