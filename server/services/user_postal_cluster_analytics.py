...
def _geometry_centroid(value: Any) -> dict[str, float] | None:
    """
    Calcule un centroïde robuste à partir de plusieurs formats :
    - {"latitude": ..., "longitude": ...}
    - {"type": "Point", "coordinates": [lon, lat]}
    - GeoJSON Feature / FeatureCollection / Polygon / MultiPolygon
    - listes de coordonnées imbriquées

    Ne dépend pas de l'ancien helper GeoJSON, afin d'éviter les erreurs
    sur les nouveaux centroïdes CP générés localement.
    """
    if value is None:
        return None
    ...
    def add_lon_lat(lon_value, lat_value) -> None:
        try:
            lon = float(lon_value)
            lat = float(lat_value)
        except (TypeError, ValueError):
    ...
    def walk(node: Any) -> None:
        ...
        if isinstance(node, dict):
            if "latitude" in node and "longitude" in node:
                add_lon_lat(node.get("longitude"), node.get("latitude"))
                return

            if node.get("type") == "Point" and isinstance(node.get("coordinates"), list):
                coords = node.get("coordinates") or []
                if len(coords) >= 2:
                    add_lon_lat(coords[0], coords[1])
                return

            if "centroid" in node:
                walk(node.get("centroid"))
            if "center" in node:
                walk(node.get("center"))
            if "geometry" in node:
                walk(node.get("geometry"))
            if "feature_collection" in node:
                walk(node.get("feature_collection"))
            if "geojson" in node:
                walk(node.get("geojson"))
            if "features" in node:
                walk(node.get("features"))
...
def _load_postal_centroids() -> dict[str, dict[str, Any]]:
    """
    Charge les centroïdes postaux.

    Priorité :
    1. géométries de consumption_postal_areas.json ;
    2. fallback multi-MLC depuis professional_enrichment.latitude/longitude.
    """
    centroids: dict[str, dict[str, Any]] = {}

    if POSTAL_AREAS_PATH.exists():
        try:
            raw = json.loads(POSTAL_AREAS_PATH.read_text(encoding="utf-8"))
        except Exception:
            raw = {}
        ...
        for postal_code, area in areas.items():
            ...
            centroid = _geometry_centroid(
                area.get("centroid")
                or area.get("center")
                or area.get("geometry")
                or area.get("feature_collection")
                or area.get("geojson")
                or area
            )
            if not centroid:
            ...
            centroids[cleaned_postal_code] = {
                ...
                "vertex_count": centroid.get("vertex_count", 0),
                "feature_count": _safe_int(area.get("feature_count"), 0),
                "source": "postal_area_geometry",
            }

    # Fallback multi-MLC : on dérive un centroïde depuis les coordonnées pros.
    with _connect() as conn:
        rows = conn.execute("""
            SELECT
                REPLACE(REPLACE(TRIM(zip), ' ', ''), char(8239), '') AS postal_code,
            ...
            GROUP BY REPLACE(REPLACE(TRIM(zip), ' ', ''), char(8239), '')
        """).fetchall()
    ...
    for row in rows:
        ...
        # On laisse les géométries officielles prioritaires, mais on complète
        # tous les codes postaux absents du fichier GeoJSON.
        centroids.setdefault(postal_code, {
            "postal_code": postal_code,
            "longitude": longitude,
            "latitude": latitude,
            "vertex_count": 0,
            "feature_count": 0,
            "source": "professional_enrichment_centroid",
            "professional_count": _safe_int(row["professional_count"], 0),
        })
...
def _table_rows(
    ...
    *,
    ...
    rows = conn.execute(
        ...
        professional_enrichment_unified AS (
          SELECT
            professional_ref,
            NULLIF(TRIM(zip), '') AS raw_postal_code,
            NULLIF(TRIM(city), '') AS city_label
          FROM professional_enrichment
          ...
          SELECT
            professional_ref,
            NULLIF(TRIM(COALESCE(cyclos_zip, zip)), '') AS raw_postal_code,
            NULLIF(TRIM(COALESCE(cyclos_city, city)), '') AS city_label
          FROM odoo_professional_enrichment
          WHERE professional_ref NOT IN (
            SELECT professional_ref FROM professional_enrichment
          )
        ),
        professional_zip AS (
          SELECT
            professional_ref,
            REPLACE(TRIM(raw_postal_code), ' ', '') AS postal_code,
            MAX(city_label) AS city_label
          FROM professional_enrichment_unified
          WHERE NULLIF(TRIM(raw_postal_code), '') IS NOT NULL
          GROUP BY professional_ref, REPLACE(TRIM(raw_postal_code), ' ', '')
        ),
        all_candidate_zip AS (
          SELECT postal_code FROM individual_zip
          UNION
          SELECT postal_code FROM professional_zip
        ),
        eligible_zip AS (
          SELECT
            candidate.postal_code,
            COALESCE(
              MAX(individual_zip.city_label),
              MAX(professional_zip.city_label)
            ) AS city_label,
            COUNT(DISTINCT individual_zip.pseudonym) AS individual_count
          FROM all_candidate_zip candidate
          LEFT JOIN individual_zip
            ON individual_zip.postal_code = candidate.postal_code
          LEFT JOIN professional_zip
            ON professional_zip.postal_code = candidate.postal_code
          GROUP BY candidate.postal_code
          HAVING COUNT(DISTINCT individual_zip.pseudonym) >= ?
             OR COUNT(DISTINCT professional_zip.professional_ref) > 0
        ),
...
def _transaction_period_bounds(conn: sqlite3.Connection) -> tuple[str | None, str | None]:
    row = conn.execute("""
        SELECT
            MIN(substr(date, 1, 10)) AS start_date,
            MAX(substr(date, 1, 10)) AS end_date
        FROM transactions
...
def get_user_postal_clusters(
    *,
    ...
    with _connect() as conn:
        ...
        period_source = "balance_windows"

        if not effective_start or not effective_end:
            fallback_start, fallback_end = _transaction_period_bounds(conn)
            effective_start = start or fallback_start
            effective_end = end or fallback_end
            period_source = "transactions"
