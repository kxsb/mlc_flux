...
DEFAULT_ACCOUNT_TYPE = "compteparticulier"
...
def _configured_account_type() -> str:
    ...
    explicit = str(balances.get("individual_account_type_internal_name") or "").strip()
    if explicit:
        return explicit

    actor_classification = profile.get("actor_classification") or {}
    names = actor_classification.get("private_actor_type_internal_names") or []
    for item in names:
