...
def create_session_token():
    cyclos_config = get_cyclos_config()
    base_url = cyclos_config.base_url
    username = cyclos_config.username
    password = cyclos_config.password

    if not cyclos_config.is_complete:
        raise ValueError(
            "Configuration Cyclos incomplète pour la MLC active "
            f"{cyclos_config.mlc_id!r}. Variables attendues : "
            f"MLC_{cyclos_config.mlc_id.upper().replace('-', '_')}_CYCLOS_BASE_URL, "
            f"MLC_{cyclos_config.mlc_id.upper().replace('-', '_')}_CYCLOS_USERNAME, "
            f"MLC_{cyclos_config.mlc_id.upper().replace('-', '_')}_CYCLOS_PASSWORD."
        )
