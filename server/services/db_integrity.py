...
REQUIRED_INDEXES: dict[str, dict[str, dict[str, Any]]] = {
    ...
    "transaction_semantics": {
        "idx_transaction_semantics_cyclos_id": {
            "unique": False,
        },
        "idx_transaction_semantics_date": {
            "unique": False,
        },
        "idx_transaction_semantics_operation_kind": {
            "unique": False,
        },
...
EXPECTED_COLUMN_CONTRACTS: dict[str, dict[str, dict[str, Any]]] = {
    ...
    "transaction_semantics": {
        "transaction_key": {"type": "TEXT", "pk": 1},
        "cyclos_id": {"type": "TEXT"},
        "transaction_number": {"type": "TEXT"},
        "date": {"type": "TEXT", "notnull": True},
        "from_label": {"type": "TEXT"},
        "to_label": {"type": "TEXT"},
        "amount": {"type": "REAL"},
        "type_label": {"type": "TEXT"},
        "group_label": {"type": "TEXT"},
        "from_actor_family": {"type": "TEXT", "notnull": True, "default": "'unknown'"},
        "to_actor_family": {"type": "TEXT", "notnull": True, "default": "'unknown'"},
        "from_account_medium": {"type": "TEXT", "notnull": True, "default": "'unknown'"},
        "to_account_medium": {"type": "TEXT", "notnull": True, "default": "'unknown'"},
        "operation_kind": {"type": "TEXT", "notnull": True, "default": "'unknown'"},
        "monetary_circuit": {"type": "TEXT", "notnull": True, "default": "'unknown'"},
        "is_economic_activity": {"type": "INTEGER", "notnull": True, "default": "0"},
        "is_monetary_supply": {"type": "INTEGER", "notnull": True, "default": "0"},
        "is_monetary_exit": {"type": "INTEGER", "notnull": True, "default": "0"},
        "is_paper_operation": {"type": "INTEGER", "notnull": True, "default": "0"},
        "is_bonus_operation": {"type": "INTEGER", "notnull": True, "default": "0"},
        "is_regularization": {"type": "INTEGER", "notnull": True, "default": "0"},
        "confidence": {"type": "TEXT", "notnull": True, "default": "'low'"},
        "reason": {"type": "TEXT"},
        "classifier_version": {"type": "TEXT", "notnull": True},
        "computed_at": {"type": "TEXT", "notnull": True},
    },
    "sync_state": {
        ...
        coverage_sample_rows = conn.execute(
            ...
            mismatch_rows AS (
                ...
                LEFT JOIN window_dates w
                 ...
                 AND (
                      w.covered_date = b.balance_date
                      OR DATE(w.covered_date, '+1 day') = b.balance_date
                 )
...
def _check_transaction_semantics_integrity(
    conn: sqlite3.Connection,
    report: dict[str, Any],
) -> None:
    integrity: dict[str, Any] = {
        ...
        "semantics_count": None,
        "coverage_delta": None,
        "classifier_versions": [],
        "unknown_count": None,
        "low_confidence": [],
        "legacy_economic_parity": {},
    }

    report["transaction_semantics_integrity"] = integrity
    ...
    if not _table_exists(conn, "transaction_semantics"):
        _append_issue(
            report,
            severity="error",
            code="transaction_semantics.missing_table",
            message="La table transaction_semantics est absente.",
        )
        return

    integrity["available"] = True

    transactions_count = int(_scalar(conn, "SELECT COUNT(*) FROM transactions") or 0)
    semantics_count = int(_scalar(conn, "SELECT COUNT(*) FROM transaction_semantics") or 0)
    coverage_delta = transactions_count - semantics_count

    integrity["transactions_count"] = transactions_count
    integrity["semantics_count"] = semantics_count
    integrity["coverage_delta"] = coverage_delta

    if coverage_delta != 0:
        _append_issue(
            report,
            severity="error",
            code="transaction_semantics.coverage_mismatch",
            message="La couverture transaction_semantics ne correspond pas à transactions.",
            details={
    ...
    versions = [
        ...
        for row in conn.execute(
            """
            SELECT classifier_version, COUNT(*) AS nb
            FROM transaction_semantics
            GROUP BY classifier_version
            ORDER BY nb DESC, classifier_version
            """
        ).fetchall()
    ]
    integrity["classifier_versions"] = versions

    if semantics_count > 0 and len(versions) != 1:
        _append_issue(
            report,
            severity="warning",
            code="transaction_semantics.multiple_classifier_versions",
            message="Plusieurs versions de classifieur transaction_semantics coexistent.",
            details=versions,
        )
    ...
    unknown_count = int(
        _scalar(
            ...
            SELECT COUNT(*)
            FROM transaction_semantics
            WHERE operation_kind = 'unknown'
               OR monetary_circuit = 'unknown'
            """,
        )
        or 0
    )
    integrity["unknown_count"] = unknown_count

    if unknown_count:
        _append_issue(
            report,
            severity="warning",
            code="transaction_semantics.unknown_operations",
            message="Des transactions restent classées en unknown.",
    ...
    low_confidence_rows = [
        {
            ...
            "monetary_circuit": str(row["monetary_circuit"]),
        ...
        for row in conn.execute(
            ...
            GROUP BY operation_kind, monetary_circuit, confidence, reason
            ORDER BY nb DESC, volume DESC
            """
        ).fetchall()
    ]
    integrity["low_confidence"] = low_confidence_rows

    unexpected_low_confidence = [
        row
        for row in low_confidence_rows
        if row["operation_kind"] != "individual_transfer"
    ]
    if unexpected_low_confidence:
        _append_issue(
            report,
            severity="warning",
            code="transaction_semantics.low_confidence_unexpected",
            message="Des opérations transaction_semantics restent en faible confiance hors transferts U→U.",
            details=unexpected_low_confidence,
        )

    legacy_economic_row = conn.execute(
        """
        WITH legacy AS (
          SELECT COUNT(*) AS nb, ROUND(SUM(amount), 2) AS volume
          FROM transactions
          WHERE (
            (
              (from_label LIKE 'U_%' OR from_label LIKE 'UD_%' OR from_label LIKE 'P____')
              AND to_label LIKE 'P____'
            )
            OR (
              from_label LIKE 'P____'
              AND (to_label LIKE 'P____' OR to_label LIKE 'U_%' OR to_label LIKE 'UD_%')
            )
          )
        ),
        pivot AS (
          SELECT COUNT(*) AS nb, ROUND(SUM(amount), 2) AS volume
          FROM transaction_semantics
          WHERE operation_kind = 'economic_payment'
        )
        SELECT
          COALESCE(legacy.nb, 0) AS legacy_nb,
          COALESCE(pivot.nb, 0) AS pivot_nb,
          COALESCE(legacy.volume, 0) AS legacy_volume,
          COALESCE(pivot.volume, 0) AS pivot_volume,
          COALESCE(legacy.nb, 0) - COALESCE(pivot.nb, 0) AS nb_delta,
          ROUND(COALESCE(legacy.volume, 0) - COALESCE(pivot.volume, 0), 2) AS volume_delta
        FROM legacy, pivot
        """
    ).fetchone()

    if legacy_economic_row:
        parity = {
            "legacy_nb": int(legacy_economic_row["legacy_nb"]),
            "pivot_nb": int(legacy_economic_row["pivot_nb"]),
            "legacy_volume": float(legacy_economic_row["legacy_volume"] or 0),
            "pivot_volume": float(legacy_economic_row["pivot_volume"] or 0),
            "nb_delta": int(legacy_economic_row["nb_delta"]),
            "volume_delta": float(legacy_economic_row["volume_delta"] or 0),
        }
    else:
        parity = {}

    integrity["legacy_economic_parity"] = parity

    if parity and (parity["nb_delta"] != 0 or abs(parity["volume_delta"]) > 0.01):
        _append_issue(
            report,
            severity="warning",
            code="transaction_semantics.legacy_economic_parity_mismatch",
            message="L'activité économique legacy ne correspond pas au pivot transaction_semantics.",
            details=parity,
        )
