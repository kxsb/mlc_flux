...
from server.services.cyclos_professional_daily_balances import (
    ProfessionalDailyBalanceBackfillAlreadyRunning,
    professional_balance_backfill_lock,
    run_daily_balance_backfill,
)


SYNC_NAME = "cyclos_professional_daily_balances"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Archive les soldes quotidiens des professionnels depuis Cyclos "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Limite optionnelle du nombre de professionnels traités, pour test.",
    )
    parser.add_argument(
        "--max-windows-per-professional",
        type=int,
        default=None,
        help="Limite optionnelle du nombre de fenêtres traitées par professionnel, pour test.",
    )
    parser.add_argument(
...
def _print_progress(event):
    ...
    ref = event.get("professional_ref") or "P?"
    label = event.get("professional_label") or ref

    if stage == "subject_start":
        print(
            f"[{event['subject_index']:>3}/{event['subjects_total']}] "
            f"{ref} — démarrage | {label}",
            flush=True,
        )
        return
...
def run_sync(
    *,
    ...
    with app.app_context():
        ...
        with professional_balance_backfill_lock():
            start_message = (
                f"running — archive soldes professionnels journaliers "
                f"{date_from}→{date_to}"
            )
            ...
            result = run_daily_balance_backfill(
                date_from=date_from,
                date_to=date_to,
                limit_professionals=limit_professionals,
                max_windows_per_professional=max_windows_per_professional,
            ...
            message = (
                ...
                f"{result['subjects_total']} professionnel(s) parcouru(s)."
            )
            ...
            print()
            print("CYCLOS PROFESSIONAL DAILY BALANCES SYNC TERMINEE")
            print(f"- période demandée : {date_from} → {date_to}")
            print(f"- professionnels parcourus : {result['subjects_total']}")
            print(f"- fenêtres / professionnel : {result['windows_per_subject']}")
...
if __name__ == "__main__":
    ...
    except ProfessionalDailyBalanceBackfillAlreadyRunning as exc:
        app = create_app()
        with app.app_context():
            init_db()
