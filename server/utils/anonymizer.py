...
def strip_invisible_text_chars(value):
    """
    Supprime les caractères invisibles parasites hérités d'exports/API
    (BOM, zero-width, marques de formatage Unicode) sans retirer les accents.
    """
    if value is None:
        return value
...
def clean_professional_label(display):
    ...
    en conservant le nom commercial complet, y compris s'il contient
    lui-même des tirets.
    """
    display = strip_invisible_text_chars(display)
    display = normalize_spaces(display)
    ...
    # Cas déjà canonique : P0008 - Nom complet - avec tirets éventuels
    match = re.match(rf"^\s*{re.escape(code)}\s*[-–—:]\s*(.+?)\s*$", display)
    if match:
        name = normalize_spaces(match.group(1))
        return f"{code} - {name}" if name else code

    # Cas inversé ou enrichi : Nom - P0008 - Titulaire
    without_code = re.sub(rf"\b{re.escape(code)}\b", "", display)
    without_code = normalize_spaces(without_code)
    without_code = re.sub(r"^\s*[-–—:]\s*", "", without_code)
    without_code = re.sub(r"\s*[-–—:]\s*$", "", without_code)
    without_code = normalize_spaces(without_code)

    return f"{code} - {without_code}" if without_code else code
