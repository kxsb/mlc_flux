...
def _resolve_pilotage_timeseries_period(
    cur,
    requested_start,
    requested_end,
    period,
    ...
    data_min = row["min_date"]
    data_max = row["max_date"]

    requested_start_iso = _pilotage_iso_date(requested_start)
    requested_end_iso = _pilotage_iso_date(requested_end)

    effective_start = max(data_min, requested_start_iso) if requested_start_iso else data_min
    effective_end = min(data_max, requested_end_iso) if requested_end_iso else data_max
...
def _fetch_pilotage_timeseries_monthly_rows(cur, effective_start, effective_end):
    """
    Renvoie les lignes mensuelles nécessaires à _build_pilotage_monthly_item.
    ...
    if rows:
        return rows, "pilotage_holdings_daily_cache"
...
def monetary_indicators_pilotage_timeseries():
    ...
    try:
        period = _resolve_period(cur, requested_start, requested_end)
        period = _resolve_pilotage_timeseries_period(
            cur,
            requested_start,
            requested_end,
            period,
    ...
    monthly_rows, monthly_source = _fetch_pilotage_timeseries_monthly_rows(
        cur,
        period["effective_start"],
        period["effective_end"],
    )
