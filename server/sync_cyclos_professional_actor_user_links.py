...
from server.services.cyclos_professional_actor_user_links import (
    sync_professional_actor_user_links,
)


SYNC_NAME = "cyclos_professional_actor_user_links"
...
def _print_progress(event):
    ...
    if stage == "month_done":
        print(
            ...
            f"{event['candidate_professional_ref_count']} pro(s) détecté(s) | "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Synchronise le mapping technique Pxxxx ↔ actor.id ↔ user.id "
            "des professionnels depuis les transactions Cyclos bulk."
        )
    )
...
def run_sync(date_from, date_to):
    ...
    with app.app_context():
        ...
        result = sync_professional_actor_user_links(
            date_from=date_from,
            date_to=date_to,
        ...
        message = (
            f"{result['stored_total']} lien(s) professionnel(s) stocké(s) au total ; "
            f"{result['created']} créé(s), "
            f"{result['updated']} mis à jour ; "
            f"{result['candidate_professional_ref_count']} référence(s) P détectée(s) "
        ...
        print()
        print("CYCLOS PROFESSIONAL ACTOR USER LINKS SYNC OK")
        print(f"- période : {date_from} → {date_to}")
        print(f"- transactions vérifiées : {result['transactions_fetched']}")
        print(f"- références P détectées : {result['candidate_professional_ref_count']}")
        print(f"- preuves P↔actor↔user : {result['candidate_evidences']}")
        print(f"- liens créés : {result['created']}")
        print(f"- liens mis à jour : {result['updated']}")
        print(f"- liens inchangés : {result['unchanged']}")
