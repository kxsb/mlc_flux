...
const MLCFLUX_MLC_CONTEXT = {
  ...
  name: "La Gonette",
  shortName: "Gonette",
  currencyName: "Gonette",
  currencySymbol: "G",
};


function syncMlcInstanceThemeAttribute(mlcIdOverride = null) {
  const candidates = [
    mlcIdOverride,
    ...
    appState?.mlcId,
    appState?.selectedMlcId,
    appState?.activeInstanceId,
    appState?.currentInstanceId,
    appState?.currentMlc?.id,
    appState?.currentMlc?.mlc_id,
    appState?.mlc?.id,
    appState?.mlc?.mlc_id,
    appState?.mlcProfile?.id,
    appState?.activeMlcProfile?.id,
    appState?.currentMlcProfile?.id,
    appState?.professionalConsumptionMap?.mlc_id,
    appState?.stats?.mlc_id,
    appState?.periodBounds?.mlc_id,
  ];

  const mlcId = candidates
...
async function syncMlcInstanceThemeAttributeFromApi() {
  try {
    ...
    const mlcId = (
      ...
      || payload?.current_mlc?.id
      || payload?.current_mlc?.mlc_id
      || payload?.mlc?.id
      || payload?.mlc?.mlc_id
    );

    syncMlcInstanceThemeAttribute(mlcId);
  } catch (error) {
    syncMlcInstanceThemeAttribute();
  }
}

function normalizeMlcProfile(profile) {
  if (!profile || typeof profile !== "object") {
    return MLCFLUX_MLC_CONTEXT;
  }

  MLCFLUX_MLC_CONTEXT.id = profile.id || MLCFLUX_MLC_CONTEXT.id;
  MLCFLUX_MLC_CONTEXT.name = profile.name || MLCFLUX_MLC_CONTEXT.name;
  MLCFLUX_MLC_CONTEXT.shortName = profile.short_name || profile.shortName || MLCFLUX_MLC_CONTEXT.shortName;
  MLCFLUX_MLC_CONTEXT.currencyName = profile.currency_name || profile.currencyName || MLCFLUX_MLC_CONTEXT.currencyName;
  MLCFLUX_MLC_CONTEXT.currencySymbol = profile.currency_symbol || profile.currencySymbol || MLCFLUX_MLC_CONTEXT.currencySymbol;

  return MLCFLUX_MLC_CONTEXT;
}

function getMlcName() {
  return MLCFLUX_MLC_CONTEXT.name || "monnaie locale";
}

function getMlcShortName() {
  return MLCFLUX_MLC_CONTEXT.shortName || getMlcName();
}

function getCurrencyName() {
  return MLCFLUX_MLC_CONTEXT.currencyName || "monnaie locale";
}
...
function replaceMlcTokens(text) {
  ...
  return String(text)
    .replaceAll("La Gonette numérique", `${getMlcName()} numérique`)
    .replaceAll("la Gonette numérique", `${getMlcName()} numérique`)
    .replaceAll("La Gonette", getMlcName())
    .replaceAll("la Gonette", getMlcName())
    .replaceAll("Gonettes", getCurrencyName())
    .replaceAll("gonettes", getCurrencyName().toLowerCase())
    .replaceAll("Gonette", getCurrencyName())
    .replaceAll("gonette", getCurrencyName().toLowerCase());
}

function formatMlcAmount(value, options = {}) {
...
function escapeMonetaryHtml(value) {
  return String(value ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function monetaryConfidenceLabel(confidence) {
  ...
  return "confiance non qualifiée";
}

function buildAdaptiveMonetaryWarningsHtml(warnings) {
  ...
  return `
    <div class="monetary-period-warning">
      ${items.map((warning) => `
        <div>${escapeMonetaryHtml(warning)}</div>
      `).join("")}
    </div>
  `;
}

function adaptiveMonetaryValue(item, keys) {
  ...
  return (Array.isArray(items) ? items : []).map((item) => {
    ...
    return `
      <tr>
        <td><strong>${escapeMonetaryHtml(item.year)}</strong></td>
        <td>${gonettes(total || 0)}</td>
        <td>${digital === null ? "—" : gonettes(digital)}</td>
        <td>${paper === null ? "—" : gonettes(paper)}</td>
        <td>${item.digital_guarantee === null || item.digital_guarantee === undefined ? "—" : accountingEuros(item.digital_guarantee)}</td>
        <td>${item.paper_guarantee === null || item.paper_guarantee === undefined ? "—" : accountingEuros(item.paper_guarantee)}</td>
      </tr>
    `;
  }).join("");
}

function renderAdaptiveMonetaryIndicators(host, payload) {
  ...
  if (!available || !latest) {
    host.innerHTML = `
      <div class="card monetary-empty-card">
        ...
        <p>Aucune donnée de masse monétaire n’est disponible pour cette instance.</p>
        ${buildAdaptiveMonetaryWarningsHtml(payload?.warnings)}
      </div>
    `;
    return;
  }
  ...
  host.innerHTML = `
    <section class="card monetary-intro-card">
      ...
      <div class="monetary-reference-line">
        Source :
        <strong>${escapeMonetaryHtml(sourceLabel)}</strong>
        · <strong>${escapeMonetaryHtml(confidenceLabel)}</strong>
      </div>
    ...
    <section class="card monetary-yearly-card">
      <div class="activity-flow-overview-header">
...
async function renderMonetaryIndicatorsTab() {
  ...
  try {
    const adaptiveEndpoint = await getAdaptiveMonetaryIndicatorsEndpoint();
    const adaptivePayload = await apiGet(adaptiveEndpoint);
    if (adaptivePayload?.available) {
      renderAdaptiveMonetaryIndicators(host, adaptivePayload);
      return;
    }
  } catch (error) {
    console.warn("Impossible de charger les indicateurs monétaires adaptatifs.", error);
  }
...
function buildPilotageHoldingsMassCompositionChartConfig(items) {
  ...
  const hasNumericMass = items.some((item) => Number(item.average_numeric_mass || 0) > 0);
  ...
  const datasets = hasNumericMass
    ? [
        ...
        {
          label: "Reste non ventilé / technique",
    ...
    : [
        {
          label: "Particuliers",
          data: items.map((item) => moneyValue(item.average_positive_user_stock)),
          stack: "observedHoldings"
        },
        {
          label: "Professionnels du réseau",
          data: items.map((item) => moneyValue(item.average_positive_professional_network_stock)),
          stack: "observedHoldings"
        },
        {
    ...
    options: {
        ...
        tooltip: {
          callbacks: {
            label(context) {
              ...
              if (hasNumericMass) {
                return `${label} : ${formatPilotageChartPercent(value)}`;
              }
      ...
      scales: {
        ...
        y: {
          stacked: true,
          beginAtZero: true,
          max: hasNumericMass ? 100 : undefined,
          title: {
            display: true,
            text: hasNumericMass
              ? "Part de la masse numérique moyenne (%)"
              : "Soldes positifs moyens observés (G)"
          },
          ticks: {
            callback(value) {
              return hasNumericMass
                ? formatPilotageChartPercent(value)
                : gonettes(value);
            }
          }
        }
      ...
      <section class="pilotage-tab-panel hidden" data-pilotage-panel="holdings">
        <section class="card pilotage-section-card pilotage-holdings-overview-card">
          ...
          <div class="pilotage-metric-grid pilotage-holdings-category-grid">
            ...
            <article class="pilotage-metric-card pilotage-holdings-category-card" data-pilotage-help="holdingsAverageGonetteBusinessAccountsStock">
            ...
            <article class="pilotage-metric-card pilotage-holdings-category-card" data-pilotage-help="holdingsUnventilatedNumericMass">
              <span>Reste non ventilé / technique</span>
              <strong>${
                holdingsReference.average_unventilated_numeric_mass !== null
                && holdingsReference.average_unventilated_numeric_mass !== undefined
                  ? gonettes(holdingsReference.average_unventilated_numeric_mass)
                  : "—"
              }</strong>
              <small>
                ${
                  holdingsReference.average_unventilated_numeric_mass_share_of_numeric_mass !== null
                  && holdingsReference.average_unventilated_numeric_mass_share_of_numeric_mass !== undefined
                    ? formatPilotagePercent(holdingsReference.average_unventilated_numeric_mass_share_of_numeric_mass)
...
function renderProfessionalConsumptionMapCard(consumptionMapSummary = null) {
  ...
  // CARTO_UP002B_TEXT_SYNC — le bloc Lecture décrit le rendu courant, pas seulement le payload complet.
  const consumptionMapCurrentRenderPayload =
    typeof buildProfessionalConsumptionMapVisualPayload === "function"
      ? buildProfessionalConsumptionMapVisualPayload(consumptionMapPayload)
      : consumptionMapPayload;

  const consumptionMapCurrentRenderCoverage =
    consumptionMapCurrentRenderPayload?.coverage || consumptionMapCoverage || {};

  const consumptionMapCurrentRenderGeometry =
    consumptionMapCurrentRenderPayload?.geometry || consumptionMapGeometry || {};

  const consumptionMapCurrentRenderVisual =
    consumptionMapCurrentRenderPayload?.visual || {};

  const consumptionMapCurrentRenderModeLabel =
    consumptionMapCurrentRenderVisual.mode === "source-focus"
      ? `Mode C · Focus zone source${
          consumptionMapCurrentRenderVisual.selected_source_postal_code
            ? ` ${consumptionMapCurrentRenderVisual.selected_source_postal_code}`
            : ""
        }`
      : `Mode A · Vue d’ensemble${
          consumptionMapCurrentRenderCoverage.visual_overview_limit
            ? ` · top ${formatProfessionalSummaryInteger(consumptionMapCurrentRenderCoverage.visual_overview_limit)} faisceaux`
            : ""
        }`;

  const consumptionMapCurrentRouteCount =
    Number(
      consumptionMapCurrentRenderCoverage.visible_route_count
      || consumptionMapCurrentRenderVisual.route_count
      || 0
    );

  const consumptionMapCurrentTxCount =
    Number(
      consumptionMapCurrentRenderCoverage.visible_tx_count
      || consumptionMapCurrentRenderVisual.tx_count
      || 0
    );

  const consumptionMapCurrentVolume =
    Number(
      consumptionMapCurrentRenderCoverage.visible_volume
      || consumptionMapCurrentRenderVisual.volume
      || 0
    );

  const consumptionMapCurrentAreaCount =
    Number(
      consumptionMapCurrentRenderGeometry?.route_area_status_counts?.available
      || consumptionMapCurrentRenderGeometry?.visual_source_count
      || consumptionMapCurrentRenderVisual.source_count
      || 0
    );
            ...
            ? `
              <div class="professional-consumption-map-kpi-grid">
                ${renderProfessionalSummaryKpiCard(
                  ...
                  "Routes bassin postal → professionnel disponibles sur la période, avant filtrage visuel Mode A / Mode C."
                )}

                ${renderProfessionalSummaryKpiCard(
                  "Volume cartographiable",
                  euro(consumptionMapCoverage.visible_volume || 0),
                  `Payload complet disponible sur la période. Le rendu courant est détaillé sous la carte.`
                )}
                ...
                ${renderProfessionalSummaryKpiCard(
                  ...
                  "Destinations professionnelles géolocalisées disponibles avant filtrage visuel."
                )}
              </div>
              ...
              ${
                typeof renderProfessionalConsumptionMapVisualControlsHtml === "function"
                  ? renderProfessionalConsumptionMapVisualControlsHtml(consumptionMapPayload)
                  : ""
              }

              <div
                id="professionalConsumptionMapLibre"
                class="professional-consumption-maplibre"
                aria-label="Carte interactive des flux U vers professionnels"
              ></div>
              ...
              <div
                class="professional-consumption-map-note"
                data-professional-consumption-map-current-note
              >
                <strong>Lecture du rendu courant.</strong>
                ${escapeHtml(consumptionMapCurrentRenderModeLabel)}.
                Les lignes illustrent des <strong>faisceaux de consommation</strong>,
                pas des trajets individuels réels.
                Le rendu affiche
                <strong>${formatProfessionalSummaryInteger(consumptionMapCurrentRouteCount)} faisceau(x)</strong>,
                <strong>${formatProfessionalSummaryInteger(consumptionMapCurrentTxCount)} paiement(s)</strong>
                et
                <strong>${euro(consumptionMapCurrentVolume)}</strong>,
                ...
                <strong>${formatProfessionalSummaryInteger(consumptionMapCurrentAreaCount)}</strong>
                bassin(s) source représenté(s).
              </div>
            `
            : `
...
function openUserPostalClustersMapZoom(points = []) {
  openStatsChartModal(
    buildProfessionalClusterMapZoomModalHtml({
      ...
      title: "Territoires d’usage et d’acceptation",
    ...
    <div class="professional-clusters-map-pair professional-clusters-map-pair-users">
      <section class="card user-postal-clusters-overview-card">
        <div class="user-postal-clusters-overview-header">
          <div>
            <div class="stat-label">2 · Territoires d’usage et d’acceptation</div>
      ...
      <section class="card user-postal-clusters-map-card">
        <div class="cartography-map-toolbar">
          <div>
            <strong>${formatUserPostalClusterInteger(points.length)}</strong> territoire(s) cartographié(s) ·
            intensité pondérée par les données territoriales disponibles.
          </div>
        ...
        <section class="pro-flow-column pro-flow-column-in">
          ...
          <div class="pro-flow-node-stack">
            ...
            ${buildProfessionalFlowNode({
              ...
              detail: `${replaceMlcTokens("gonettes numériques créditées")}`,
...
/* GRAINE_THEME001_FIX4 — déclencheur autonome du thème MLC */
(() => {
  if (window.__MLCFluxThemeSyncFix4) return;
  window.__MLCFluxThemeSyncFix4 = true;

  const run = () => {
    if (typeof syncMlcInstanceThemeAttributeFromApi === "function") {
      syncMlcInstanceThemeAttributeFromApi();
    } else if (typeof syncMlcInstanceThemeAttribute === "function") {
      syncMlcInstanceThemeAttribute();
    }
  };
...
/* CARTO_CLUSTER_MAIN_OPT004A_SAFE_FRONTEND_LEAN_INITIAL_PAYLOAD
 * Premier chargement carto U→P allégé :
 * - compact_geometry=1
 * - include_route_timeline=0
 * - include_extra_flow_families=0
 */
function getProfessionalConsumptionMapLeanInitialQuery(baseQuery) {
  const query = String(baseQuery || "");
  ...
  const additions = [
    ...
    "include_route_timeline=0",
    "include_extra_flow_families=0",
    "source_points_mode=representative", // CARTO_CLUSTER_MAIN_OPT005A_REPRESENTATIVE_SOURCE_POINTS
    "route_endpoint_mode=compact" // CARTO_CLUSTER_MAIN_OPT006A_COMPACT_ROUTE_ENDPOINTS
  ].filter((param) => {
